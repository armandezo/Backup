 ...
 <?php $post = $posts[0]; ?>
				<header class="page-header">
					...
					<h1 class="page-title">
						...
						<?php
							if ( is_category() ) {
								printf( __( 'Artículos de la Categoría: %s', '_mm' ), '<span>' . single_cat_title( '', false ) . '</span>' );
			...
			<?php else : ?>
				...
				<article id="post-0" class="post no-results not-found">
					<header class="entry-header">
						<h1 class="entry-title"><?php _e( 'Nothing Found', '_mm' ); ?></h1>
					</header><!-- .entry-header -->

					<div class="entry-content">
						<p><?php _e( 'It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.', '_mm' ); ?></p>
						<?php get_search_form(); ?>
					</div><!-- .entry-content -->
				</article><!-- #post-0 -->
