...
function grunion_post_type_columns_filter( $cols ) {
	$cols = array(
		...
		'feedback_from'		=> __( 'From' , "mm"),
		'feedback_message'		=> __( 'Message', "mm" ),
		'feedback_date'			=> __( 'Date' , "mm")
	);
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '"><a title="';
				echo esc_attr__( 'Resore this item from the Trash', "mm" );
				...
				echo '">' . __( 'Restore' , "mm") . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently' , "mm") );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently', "mm" ) . "</a></span>";
?>
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as spam', "mm" );
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash', "mm" );
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash' , "mm") . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam' , "mm");
...
<?php
		...
		case 'feedback_date':
			echo get_the_date( __( 'Y-m-d @ g:i:s A' , "mm") );
			break;
	}
}
...
function grunion_ajax_spam() {
	...
	if ( !current_user_can("edit_page", $post_id) )
		wp_die( __( 'You are not allowed to manage this item.', "mm" ) );
	...
	if ( $_POST['make_it'] == 'spam' ) {
		...
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item out of the Trash.', "mm") );

		if ( ! wp_untrash_post($post_id) )
			wp_die( __('Error in restoring from Trash.', "mm") );

	} elseif( $_POST['make_it'] == 'trash' ) {
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item to the Trash.', "mm") );

		if ( ! wp_trash_post($post_id) )
			wp_die( __('Error in moving to Trash.', "mm") );
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages', "mm" ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash', "mm" ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam', "mm" ) . ' <span class="count">';
