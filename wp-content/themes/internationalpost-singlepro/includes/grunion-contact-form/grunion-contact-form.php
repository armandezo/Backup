...
define( 'GRUNION_PLUGIN_URL', get_template_directory_uri() . '/includes/grunion-contact-form/');
...
function contact_form_render_field( $field ) {
	...
	if ( $field['type'] == 'email' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='grunion-field-label ".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", "mm") . '</span>' : '' ) . "</label>\n";
		$r .= "\t\t<input type='text' name='".esc_attr($field_id)."' id='".esc_attr($field_id)."' value='".esc_attr($field_value)."' class='".esc_attr($field['type'])."'/>\n";
		$r .= "\t</div>\n";
	} elseif ( $field['type'] == 'textarea' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", "mm") . '</span>' : '' ) . "</label>\n";
		...
		$r .= "\t<div><label class='". ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", "mm") . '</span>' : '' ) . "</label>\n";
		...
		$r .= "\t\t". htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", "mm") . '</span>' : '' ) . "</label>\n";
		$r .= "\t\t<div class='clear-form'></div>\n";
		$r .= "\t</div>\n";
...
function contact_form_validate_field( $field ) {
	...
	if ( $field['required'] && $field['type'] == 'email' ) {
		if ( !is_email( $field_value ) ) {
			...
			$contact_form_errors->add( $field_id, sprintf( __( '%s requires a valid email address' , "mm"), $field['label'] ) );
		}
		...
		$contact_form_errors->add( $field_id, sprintf( __('%s is required', "mm"), $field['label'] ) );
	}
...
" . __( "Time:" , "mm") . " " . $time . "
" . __( "IP Address:", "mm" ) . " " . $comment_author_IP . "
" . __( "Contact Form URL:", "mm" ) . " " . get_permalink( $post->ID ) . "
...
function contact_form_init() {
	...
	register_post_type( 'feedback', array(
		'labels'	=> array(
			'name'			=> __( 'Feedbacks', "mm" ),
			'singular_name'	=> __( 'Feedback', "mm" ),
			'search_items'	=> __( 'Search Feedback', "mm" ),
			'not_found'		=> __( 'No feedback found', "mm" ),
			'not_found_in_trash'	=> __( 'No feedback found' , "mm")
		),
...
function grunion_media_button( ) {
	...
	$title = esc_attr( __( 'Add a custom form', "mm" ) );
