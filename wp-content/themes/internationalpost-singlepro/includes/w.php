...
function m3_custom_login_logo() {    echo '<style type="text/css">
     ...
     </style>';  } add_action('login_head', 'm3_custom_login_logo'); function m3_custom_login_logo_url($url) { 	return home_url('url'); }
...
echo '</div>';
	...
	function form($instance)
	{
		$title	= __( 'Visit us on Google+', "mm");
		$size	= 'badge';
		...
		echo '<p>';
			//Widgtet title
			echo sprintf('<label for="%1$s">%2$s</label>', $this->get_field_id('title'), __('Title:', "mm"));
...
class multiplesocialwidget extends WP_Widget {
	...
	function multiplesocialwidget() {
		$widget_ops = array('classname' => 'multiplesocialwidget', 'description' => __('Your social stats in sidebar.', "mm"));
		$control_ops = array('width' => 250, 'height' => 350);
		$this->WP_Widget('multiplesocialwidget', __('M3: Social Stats Widget', "mm"), $widget_ops, $control_ops);
	}
